...
(function () {
  ...
  function removeBGImages() {
    ...
    for (const item of everything) {
      if (item.style && item.id != window.cloneImageId) {
  ...
  function isWhitelisted(list = "") {
    const domains = list.split("\n");
    const { hostname } = location;
    return !!domains.find((domain) => hostname.includes(domain));
  }
  ...
  function onPageLoad(e) {
    chrome.storage.sync.get(
      ...
      function (values) {
        if (!isWhitelisted(values.whitelist) && values.blurOnDefault) {
          initialBlurAll();
        } else {
  ...
  function repositionMask(e) {
    ...
    img.style["webkitMaskPosition"] =
      e.clientX - ir.left + "px " + (e.clientY - ir.top) + "px";
  }
  ...
  document.addEventListener("DOMContentLoaded", function (e) {
    addMaskDivToPage();
    onPageLoad(e);
    var observer = new MutationObserver(onPageLoad);
